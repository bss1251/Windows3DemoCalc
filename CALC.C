...
HANDLE hInstance,hPrevInstance; /*Declaring parameters this way was required for ANSI C*/
LPSTR lpszCmdLine;
int nCmdShow;
{
	MSG msg;
	...
	if (!hPrevInstance) { /*Windows 3 and before only allow registering a window class once*/
...
LONG lParam;
{
	...
	case WM_CREATE:
		...
		break;
	case WM_COMMAND:
		if (HIWORD(wParam) == BN_CLICKED) { /*Button clicked - check ID to see which one and perform correct action*/
          ...
          immed=0;
				...
				case 106:
					...
					break;
        default:
			    if (immed < 1e24) { /*Just put some limit here to prevent insanity and overflows*/
				    immed*=10;
